...
import { colorSelected, toggled ,deleted} from '../../redux/todos/todosAction';
    ...
    <>
         <div className="flex justify-start items-center p-2 hover:bg-gray-100 hover:transition-all space-x-4 border-b border-gray-400/20 last:border-0">
            ...
            <input  onChange={() =>handleStatusChanged(id)}type="checkbox"className="opacity-0 absolute rounded-full"/>
                ...
                <img
                src={cancelImg}
                className="flex-shrink-0 w-4 h-4 ml-2 cursor-pointer"alt="Cancel" onClick={()=>handleDeleted(id)}/>
               </div>
    </>
  )
}
